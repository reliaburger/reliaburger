...
impl NodeRenewalWorker {
    ...
    /// seconds. Resolve the current leader on every attempt; never proxy a CSR
    /// through another node or guess a remote API port. Shutdown cancels waiting
    /// or network I/O; an identity save already in progress owns its completion.
    pub async fn run(
        self,
