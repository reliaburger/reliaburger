...
When serde encounters an integer, it calls `visit_u64`. When it encounters a string, it calls `visit_str`. We handle both, and reject everything else with the `expecting` message. `deserialize_any` tells serde to figure out the type from the input rather than expecting a specific one.

This pattern — enum type, custom visitor, `deserialize_any` — is the standard way to handle TOML values that can be more than one type. You'll see it again.
...
This way, any code that touches environment variables knows immediately whether a value needs decryption. The type carries the information, not a comment or a convention.
