...
- [x] **Control-plane directory and reporting robustness** — publish authenticated leader
  ...
  - [x] No silent task death, no hot spins (CP10): report/rollup workers exit cleanly and
    log when their leader-target watch closes (previously a 100%-CPU spin); cluster
    runtime tasks wrapped in `spawn_supervised` (loud log on unexpected exit; respawn
    deliberately rejected — documented in `runtime.rs`).
