...
- [ ] **C06** (P1) Reserve node-fault capacity across the cluster. Live leader-fault expiry exposed a prerequisite gossip defect: a seedless bootstrap node could forget every peer during isolation. Prerequisite fixed: retain bounded rediscovery contacts and exchange direct-peer refutation evidence after ordinary piggyback retries expire. The seedless-rejoin regression fails before the fix; all 151 gossip tests and strict Linux all-target/all-feature Clippy pass. Reservation, fencing and membership-guard changes remain under live acceptance validation.
