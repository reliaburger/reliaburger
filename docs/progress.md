...
- [ ] **C38** (P2) Finish cooperative deployment cancellation and app/job naming. Ownership repair is committed: error events no longer release a target before rollback and worker completion; slow event readers cannot pin terminal accounting, and busy errors show ID/age/phase. The blocked-cleanup regression fails before the fix, then both strategies pass; all 98 agent, four operation-store and six deploy API tests pass with strict Linux Clippy. Cross-kind admission is also fixed: conflicting config names refuse; the agent/supervisor preserve existing instance kinds, and app rollout excludes jobs. The overwrite regression fails before the fix; 99 agent, 44 supervisor and 173 configuration tests plus strict Linux Clippy pass. Cooperative cancellation remains open.
