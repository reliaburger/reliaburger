...
- [ ] **C06** (P1) Reserve node-fault capacity across the cluster. Prerequisite gossip repairs now retain bounded rediscovery contacts, repair partial membership loss even with another live neighbour, and exchange direct-peer refutation evidence after ordinary piggyback retries expire. Both rediscovery regressions fail before their fixes; all 152 gossip tests pass. Reservation, fencing and membership-guard changes have passed live acceptance and await their own commit.
