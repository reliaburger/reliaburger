...
- [ ] **C34** Finish durable ownership and confirmed cleanup for every supported test resource. The [completion plan's C34 record](plans/2026-09-17-codebase-completion-plan.md#c34--define-lease-ownership-for-the-remaining-test-resources) preserves the per-change regressions, platform results and historical format generations. Current committed formats are protocol 14/state 23 and lease schema 5.
  ...
  - [ ] Recover runtime/discovery resources created before their first adoption record, including physical process-death qualification. The [OCI ownership plan](plans/2026-09-20-oci-launch-ownership.md) records the remaining evidence and implementation order.
    ...
    - [ ] Recover owned runc launchers and rootless slirp helpers without signalling recovered PIDs.
      ...
      - [x] Integrate opt-in owned rootless Runc and slirp with pinned namespace descriptors and a network-readiness gate. Five real unprivileged Linux cases, one namespace-refusal contract and their subprocess fixture pass (3.79s), including short jobs, helper replacement, long paths, actual caller death and interrupted helper startup without late payload execution. All six rootful cases plus their fixture still pass (5.22s); 37 macOS/40 Linux selected ownership regressions and strict Clippy on both pass. Production selection and discovery/Apple recovery remain open.
    ...
    - [ ] Reconcile discovery/egress state before adoption and cover Apple Container's in-flight CLI creation window.
      ...
      - [ ] Retain and recover kernel discovery/egress policy across actual Bun death. The original isolated-cgroup SIGKILL regression reproduced lost enforcement. Pre-start namespace binding, discovery cleanup, host-reboot recovery, production kernel selection and actual Bun/upgrade qualification remain open.
        ...
        - [ ] Finish durable service/backend ownership and per-instance rollout updates; positively confirm destination-grant removal before retirement. Frozen-map refusal, partial programming and actual Bun death must retain original obligations. Source ownership is tracked separately above.
        - [ ] Qualify init-container cgroup lifetime before parent startup and cancellation/retry boundaries with real runtime ownership.
        - [ ] Reproduce and correct bare-name destination firewall IDs across namespaces, including remote service entries and hash-collision handling (inspected, not yet reproduced).
