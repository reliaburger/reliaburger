...
- [ ] **C34** (P2) Define lease ownership for the remaining test resources. Token ownership is complete: atomic scoped issuance, expiry bounds, exact-owner admission and fingerprint-fenced cleanup through client/leader death. Five API tests (2.36s), 74 Raft tests, 13 local lease tests, actual HTTPS catalogue/cleanup (12.33s) and three-node leader-failure cleanup (17.60s) pass, together with compatibility, 17 client tests and strict Linux/macOS Clippy. Compatible join/upgrade fixtures now follow CURRENT after CI caught old literals; the full Linux checkpoint passes 3,263 tests (19 explicit gates, 39.69s). Protocol 5/state 4 and lease schema 2 require fresh development clusters. Pre-first-run cron retirement is repaired separately (104 agent tests, 12.57s; strict Linux/macOS Clippy); unsupported manifests now refuse before unleased mutation, and the API rejects unleased test jobs (14 context and 48 lease-filtered tests, one explicit gate; strict Linux/macOS Clippy). Successful and explicitly stopped job retries now stay stopped: both failing-first command-channel regressions pass (5.03s), alongside 44 supervisor tests, 104 agent tests and strict Linux/macOS Clippy. Cron firings now share normal deployment ownership and cancellation; local stops return 409 while a worker owns the target, and overlapping deploys refuse. The blocked-create regression fails before the fix; three job lifecycle, 104 agent and 114 API tests plus strict Linux/macOS Clippy pass. Reapplying a job without a schedule now removes only its namespace’s previous cron registration; the failing-first command regression, 105 agent tests (12.50s), three lifecycle tests (5.03s) and strict Linux Clippy pass. Local stops now require observed exit after force-kill, bound runtime calls and preserve adoption records on uncertainty. Three HTTP regressions fail before the fix; all seven lifecycle tests (12.21s), 105 agent tests (11.79s), 48 lease-filtered tests (one explicit gate) and strict Linux Clippy pass. ProcessGrill now refuses unverified adopted signals, propagates signal/wait failures and bounds forced-exit waits. Process/runc status inspection and the shared adopted-process poller preserve errors; rootless shutdown propagates them. Four ProcessGrill regressions fail first; 265 Linux runtime tests (13 explicit gates, 7.57s), seven job-lifecycle tests (12.03s), 106 agent tests (11.73s), native process tests and strict Linux/macOS Clippy pass. Runc stop/kill now check CLI outcomes and retain the launcher until observed exit. Three controlled regressions fail first; real adoption catches and verifies the rootful startup race. Startup waits for a running PID or completed rootful job, preserving both zero/non-zero job results. Nine ordinary runtime tests, seven rootful acceptance cases (26.35s), non-root port adoption (3.85s, after the CI AppArmor prerequisite) and strict Linux/macOS Clippy pass. Runc cleanup now propagates teardown errors, requires normal rootfs unmount and confirms OCI/launcher/network absence before retiring ownership. Failed/cancelled cleanup retries without publishing Stopped or losing an adopted exit code. Deletion, exit-code and real busy-overlay regressions fail first; 271 runtime tests (13 explicit gates, 7.64s), seven rootful cases (26.49s), non-root port recovery (3.70s), the actual TLS/runc secret/config catalogue with three confirmed cleanups (62.76s) and strict Linux/macOS Clippy pass. Placement reconciliation now persists Pending ownership before queueing deployment and retains it through interrupted work, withdrawal and cleanup retries. Restart inventory invalidates convergence without forgetting ownership; invalid checkpoints refuse. This changes the checkpoint schema to 2 and durable state to generation 5 (protocol 5, lease schema 2 unchanged). The ordering regression fails first; 61 cluster tests (4.15s), the final failure/restart fixture (6.17s), five compatibility tests, all nine live multi-node placement cases (135.38s), the full Linux library checkpoint (3,299 passed, 19 explicit gates, 54.89s) and strict Linux/macOS Clippy pass. Full process-tree/atomic ownership, initial runtime-record crash recovery and cluster-wide retirement evidence still need qualification; placement response bodies and retirement replies now share bounded operation deadlines, retain ownership on uncertainty and permit later owners to progress. Both stalled-I/O regressions fail first. All 63 cluster unit tests (10.32s), real three-node stop/non-resurrection (13.14s) and strict Linux/macOS Clippy pass. Normal retirement plus bounded observation retries also pass the actual TLS/runc three-case cleanup qualification (65.36s). Normal lease cleanup and removed cluster placements now use an internal Retire command: confirmed exit precedes releasing instance inventory, host ports and cached specs. The running-agent lease regression fails before the fix; all 106 agent tests (12.44s) and strict Linux Clippy pass. Cleanup confirmation now retries transient observation failures inside its existing deadline; it still requires observed namespace absence and returns Unknown on persistent failure. The recovery regression fails before the fix; all 117 testkit tests (0.54s) and strict Linux Clippy pass. Durable node-local job leases now persist before deployment, recover after Bun is killed and reclaim schedules plus confirmed runtime retirement. Reserved namespaces and scope checks prevent cross-kind ownership; node-job routes never forward or fall back to Raft. The regression fails first; five scope/routing/restart tests (5.97s), 119 testkit tests (1.23s), and actual Bun-kill recovery plus the three-case TLS/CLI catalogue (14.00s, all cleanups confirmed) pass. Protocol/state are now 6/6 and lease schema 3. The full Linux library checkpoint passes 3,306 tests (19 explicit gates, 63.99s), and both actual-binary compatibility tests pass (0.01s). Strict Linux/macOS all-target/all-feature Clippy passes. The job completion probe now requires an observed zero exit; missing evidence no longer passes. Its regression fails first, all 120 testkit tests pass (1.52s), and the actual three-case TLS/CLI job catalogue passes with confirmed cleanup (8.65s). Strict Linux/macOS Clippy passes. Job log publication now polls within the original deadline rather than failing on a temporarily empty store. The delayed-publication regression fails first; missing output stays a failure. All 122 testkit tests (1.76s) and the actual TLS/CLI job catalogue (7.48s, confirmed cleanup) pass, as does strict Linux/macOS Clippy. Registry uploads/repositories and managed volumes/mounts remain. Startup now refuses uncertain adoption, unreadable/corrupt records, wrong runtimes and port conflicts before serving; identity retirement must succeed before removing a dead owner's record. Both loss regressions fail first. All 435 Bun tests (one explicit gate, 34.34s), 12 record tests and 11 ordinary real first-run cases (17.60s) and strict Linux/macOS Clippy pass. Process/runc adoption now uses checked observation and rejects invalid PID selectors before any syscall, retaining resources on inspection errors. The invalid-record regression fails first; all 277 Linux runtime tests (13 explicit gates, 7.26s), 22 native ProcessGrill tests (0.22s), real rootful adoption/rootfs retirement (6.81s) and strict Linux/macOS Clippy pass. Apple inspection now preserves daemon/parse/identity errors, refuses created/paused adoption and establishes absence only from a successful empty inventory. The original daemon-error regression fails first. Eight native tests (10.01s), including timeout/CLI reaping, real Apple adoption/removal (1.89s), and strict native/Linux Clippy pass. Apple tests are macOS-only. Durable cron recovery is now verified: registrations, explicit retirement and pre-launch UTC-minute claims persist atomically; startup validates and restores ownership. Uncertain writes fence scheduling, and firing waits for an existing deploy worker. The agreed policy skips missed or uncertain occurrences without catch-up. Three original regressions and the registration-worker race fail before their fixes. All 446 Linux Bun tests (36.07s), 447 native Bun tests (21.25s; one explicit gate each), the actual Bun SIGKILL registration/retirement test (0.27s), both compatibility tests (0.01s) and strict Linux/macOS Clippy pass. Current protocol/state are 6/7; lease schema remains 3. Durable job execution intent and retry budgets remain open. Hosted macOS CI at `c4ec142` found EPERM when cleaning up an exited cron process; both the actual catalogue and a non-reaping kernel/runtime regression reproduce it. Stop/kill now collect completed child status before signalling and preserve zero/non-zero exits, while live-process errors propagate. All 21 native ProcessGrill tests (0.24s), the actual macOS catalogue (17.79s, three confirmed cleanups), 274 Linux runtime tests (13 explicit gates, 7.58s), seven lifecycle tests (12.03s), both real Linux job catalogue/recovery cases (13.32s) and strict Linux/macOS Clippy pass. Both hosted CI and Build & Release pass at `50fed01`. Restart cleanup now requires bounded force-kill and observed exit before recreate; uncertainty retains Pending ownership, port, record and the in-memory retry count for a later attempt. All four regressions fail first and recover after the injected fault clears. All 440 Linux Bun tests (34.83s), 441 native Bun tests (20.68s; one explicit gate each) and strict Linux/macOS Clippy pass. Create/start retry failures now keep ownership through observed cleanup and resume under backoff/budget limits; apps retain retry eligibility during backoff, and explicit stops cancel Pending retries. Four startup regressions and a Pending-stop regression fail first. All 452 Linux Bun tests (37.77s), 453 native Bun tests (21.15s; one explicit gate each), 33 lifecycle tests and strict Linux/macOS Clippy pass, including actual executable removal/restoration and a second process crash. Durable retry budgets across Bun replacement remain open. Peer API addressing is now explicit: membership publishes resolved per-node endpoints; testkit, diagnostics/trace and upgrade plans refuse missing evidence rather than reusing an entry/default port. The original HTTP regression fails first. Two API, 126 testkit and 332 Relish tests pass on both platforms; authenticated three-node reads verify distinct ports (19.96s native/18.21s Linux), with strict Linux/macOS Clippy. Upload cleanup now retains an explicit Retiring owner until file deletion and directory sync succeed. Expired/failed sessions refuse new writers; failed cleanup retries without blocking other expired uploads. The repeat-sweep regression fails first. All 229 Pickle tests and two authenticated HTTP tests pass on macOS/Linux, with strict Clippy. Upload restart recovery now claims exclusive directory ownership before any writer starts, reclaims recognised regular temporary files and refuses competing owners, unexpected entries or symlinked directories. The actual Bun SIGKILL regression and symlink regression fail before their fixes. All 25 store tests, 13 real first-run tests (33.31s native/17.97s Linux; two explicit Linux rootful gates) and strict Linux/macOS Clippy pass, including a competing-Bun refusal that preserves the live upload. All three actual Linux cluster upgrade/rollback/pause-recovery cases pass (191.46s). Lease release now waits for durable absence after HTTP 202; a still-present lease remains unknown even with an empty runtime inventory. The HTTP regression fails before the fix; all 127 testkit tests and strict Clippy pass on macOS/Linux. Normal Stop/Retire now retain ownership on identity or adoption-record removal failure; cleanup syncs both parent directories and retries. The command-channel regression fails first and verifies both faults plus recovery. Full library suites pass 3,308 macOS/3,362 Linux tests (five/19 explicit gates), with strict Clippy on both. Rolling-deploy artifact cleanup, lease-owned repositories and cluster retirement acknowledgements remain open. Node effects use C06/C10; image distribution reports uncontrolled cache state without eviction.
