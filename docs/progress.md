...
- [ ] **C34** Finish durable ownership and confirmed cleanup for every supported test resource. The [completion plan's C34 record](plans/2026-09-17-codebase-completion-plan.md#c34--define-lease-ownership-for-the-remaining-test-resources) preserves the per-change regressions, platform results and historical format generations. Current committed formats are protocol 14/state 20 and lease schema 5.
  ...
  - [ ] Recover runtime/discovery resources created before their first adoption record, including physical process-death qualification. The [OCI ownership plan](plans/2026-09-20-oci-launch-ownership.md) records the remaining evidence and implementation order.
    ...
    - [ ] Reconcile discovery/egress state before adoption and cover Apple Container's in-flight CLI creation window.
      ...
      - [ ] Retain and recover kernel discovery/egress policy across actual Bun death. The original isolated-cgroup SIGKILL regression reproduced lost enforcement. Container source identity, successor ordering, host-reboot recovery, production kernel selection and actual Bun/upgrade qualification remain open.
        ...
        - [x] Persist original per-workload egress ownership before map writes and restore it before adoption. The adoption and interrupted-cleanup regressions fail first. An explicit retired marker survives identity/adoption cleanup failure; missing owners never imply successful cleanup. All 38 physical Linux kernel tests pass (8.83s), including checkpoint-write refusal, missing enforcement and repeated frozen-map recovery. Four portable checkpoint contracts and all 206 selected agent/egress tests pass on macOS/Linux, with strict Clippy on both. Original source identity, successor ordering and production kernel selection remain separate.
        - [ ] Retire ordinary application adoption/policy artifacts before creating the next runtime generation; qualify actual crash recovery before its replacement record is published.
        - [ ] Resolve firewall and trace source cgroups from verified container identity instead of Runc's launcher PID; confirm source-binding and discovery cleanup before retirement.
