...
- [ ] **C14** (P1) Renew and hot-reload every served certificate class. Issuer-chain prerequisite fixed separately: ingress sends the leaf and its original root-signed intermediate, so root-only clients validate it. The real TLS handshake fails with UnknownIssuer before the fix, then passes (0.02s); all ten ingress TLS tests and strict Clippy on Linux Rust 1.97 and macOS Rust 1.98 pass. Cached ingress leaves now renew at their X.509 validity midpoint, recover after idle expiry and cannot outlive their issuer. The failing-first real TLS renewal test passes (17.04s), as do 12 ingress TLS and 16 CA tests. Operator file pairs now reload once per second after key/chain validity checks; the real listener survives mismatched and malformed replacements and keeps an existing HTTP connection alive (3.26s). Ingress session resumption is disabled so reconnects validate the current certificate; the real TLS 1.2/1.3 reconnect matrix fails with Resumed before the fix and passes afterwards (0.02s). Renewal/reload and strict Linux/macOS Clippy pass. API/registry/ingress TLS connections now retire within one hour, with 30 seconds for HTTP draining; the I/O deadline survives WebSocket upgrades. Three deadline regressions, real upgraded ingress retirement/reconnect (0.12s), Bun API in-flight draining (0.05s), three reload tests, seven ingress tests and strict Linux/macOS Clippy pass. Node identities now persist as private atomic snapshots with key/chain/node/serial validation and signed validity dates; 17 persistence and 29 managed-bootstrap tests pass, including failed replacement, concurrent reads, corrupt snapshots and incomplete installation. The full Linux library checkpoint passes (3,255 tests; 19 explicit privileged gates), as do 11 security and two API TLS integration tests and strict Linux/macOS Clippy. Automatic node issuance and live transport updates remain open.
