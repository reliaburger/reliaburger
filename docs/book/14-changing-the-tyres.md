...
1. **`step`** — one tick of the walk, generic over a `NodeControl` trait (probe a node, send a directive). All fourteen behaviour tests run against a mock cluster in a `HashMap`; not one of them opens a socket.
...
One honesty note up front. With four nodes and a council cap of seven, *every* node becomes a Raft voter — a genuine non-voter worker would need an eight-node harness, which is a lot of laptop for one assertion. So "worker" in these tests is a role in the upgrade *plan* (which is how the orchestrator sees roles anyway; relish assigns them from gossip flags in production). The mechanics under test — batch-then-serial ordering, quorum-gated council steps, leader-last in-place upgrade — are untouched by the distinction.
