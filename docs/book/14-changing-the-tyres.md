...
One honesty note up front. With four nodes and a council cap of seven, *every* node becomes a Raft voter — a genuine non-voter worker would need an eight-node harness, which is a lot of laptop for one assertion. So "worker" in these tests is a role in the upgrade *plan* (which is how the orchestrator sees roles anyway; relish assigns them from gossip flags in production). The mechanics under test — batch-then-serial ordering, quorum-gated council steps, the transfer dance — are untouched by the distinction.

**`rolling_upgrade_walks_workers_council_then_leader`** is the milestone test. Deploy a workload on the worker, note its pid, push the signed blob to the leader's Pickle, POST the plan, and then just *watch* `/v1/upgrade/cluster` — any node can serve it, it's replicated — recording when each node first reports `Healthy`. The assertions read like the design doc: worker first; old leader last; leadership actually moved (the node answering "I am the leader" at the end is not the one that started the run); all four nodes report v0.2.0; and the workload's pid never changed while the entire orchestration layer replaced itself underneath it.
