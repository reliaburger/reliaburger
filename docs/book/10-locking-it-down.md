...
**Rolling redeploy.** The fresh-deploy path programmed egress. The rolling-redeploy path — which creates replacement instances inline, for speed — quietly didn't, so every *redeployed* workload ran with no egress rules at all. A workload you never redeploy is enforced; the moment you ship a new version, enforcement evaporates. Fresh deploy, rolling replacement and crash restart now share the same create → program → start contract. Post-start networking only refreshes service and namespace-firewall maps. If a restart has no trustworthy cgroup path, Bun refuses it. Refusing has a catch, though: by then the replacement container already exists. Bun stops it before marking the instance failed, and if that stop fails it doesn't pretend otherwise. The instance goes back through the ordinary "restart failed, cleanup owed" path, so the next pass retires the container instead of leaving it behind with nobody responsible for it.

**Failing closed.** The original code bailed on the first error — DNS didn't resolve, a map write failed — and returned having enforced nothing. That's *fail-open*: the instance runs with unrestricted egress, the exact opposite of what `[egress] allow` asked for. Now, once we know egress is configured and eBPF is loaded, we enable enforcement *first* (an enabled flag with no allow entries denies everything) and only then open destinations one at a time. A failure anywhere after that point leaves the cgroup denying all egress, and the periodic re-resolve fills the allowlist back in once DNS recovers. Deny-by-default is only a safe default if the failure path defaults to it too. This contract starts when you declare an allowlist; an app with no `[egress]` block still has unrestricted egress. The cluster-wide default from the design remains future work.
...
There's a policy decision hiding in the failure mode. If the kernel refuses to attach connect6 (it shouldn't, the hook predates our 5.7 minimum, but hardware surprises you), what do you do with an app that has an allowlist? Loading "degraded" and enforcing v4-only means shipping the exact bug we just fixed. So the agent refuses the deploy instead: an allowlist you can trivially bypass isn't enforced, and we'd rather say so at deploy time than pretend. The scheduler receives that same live capability, but the pre-start agent check remains the final boundary in case its view is stale. The Lima test `v4_only_allowlist_no_longer_bypassed_over_ipv6` pins the regression: the same v6 connect that used to succeed must now come back `EPERM`.

### The UDP bypass

Fixing both address families still left another door. An unconnected UDP socket calls `sendto()` or `sendmsg()` with a destination on every datagram. It doesn't call `connect()`, so neither connect hook sees it. A compromised workload could send arbitrary UDP traffic despite a perfect-looking dual-stack capability.

The kernel has matching cgroup hooks for this exact case. `onion_sendmsg4` and `onion_sendmsg6` now use the same exact-address and CIDR maps as the connect programs. The node capability is true only when all four hooks are live. This distinction matters because “the eBPF programme loaded” is too vague to schedule against; a security capability has to name every path it closes.
...
Failure handling is strict because we still *can* refuse: if the maps can't be programmed, the created container is removed and the deploy fails, rather than starting a workload ahead of its policy. (A transient DNS failure is gentler: enforcement goes on with an empty allowlist, deny-all, and the re-resolver fills it in.) A `Grill` method, `honours_cgroup_path()`, gates the whole path. Root-mode runc says yes. ProcessGrill and Apple Container say no because they don't place workloads into our cgroup tree, and rootless runc can't promise ownership of it. Those runtimes now reject policy-bearing workloads. There is no safe post-start window, documented or otherwise.
