...
There's a gap in that story, though. The thing a new user actually runs is `curl … | sh`, and until something is published at an HTTPS address, nobody can run it. Promote first and test afterwards? Then the first person to try the one-line install is whoever reads the announcement. So there's a dress rehearsal. `stage.yml` takes the same candidate run and digest that promotion takes, verifies the download with the same `candidate.py` code, and publishes the unchanged files as a GitHub pre-release. The installers already accept `RELIABURGER_RELEASE_BASE_URL`, and `relish setup` redirects only that version's release URLs to it, so the staged directory behaves exactly like the real one while every version string inside still says `v0.1.0`.
...
One file sat awkwardly between the two lists: `context.lock`. Every write of `context.json` takes an advisory lock (`File::try_lock`, which is `flock` underneath) on this sibling file, and `relish local destroy` deliberately leaves it behind. That looks like tidiness gone wrong, but deleting a lock file is a classic race. Process A opens `context.lock`; we unlink it; process B opens the same *path*, gets a brand-new file, and locks it. A then locks the old, nameless inode. Both believe they hold the lock. The Apple silicon qualification run for the 0.1.0 candidate flagged the leftover, because an uninstall that says it removes what the installer made and then leaves a file behind is a bug from the user's side of the screen.

So uninstall now removes the lock when no `context.json` is left for it to guard, and it does it the careful way: take the lock, check the context didn't reappear, unlink, release. The other half of the fix lives in `lock_context`. After locking, it compares the device and inode numbers of the file it holds (`File::metadata`) with whatever the path names now (`std::fs::symlink_metadata`), through the Unix-only `MetadataExt` trait. If they differ, or the path is gone, it refuses with "busy; retry" instead of trusting a lock nobody else can see. A saved context keeps its lock, since that's still in use.
