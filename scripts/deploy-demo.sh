...
echo ""
echo "--- auto-rollback: v2 should still be running (v3 never replaced it) ---"
curl -sf http://localhost:8081/health && echo " ← v2 still healthy ✓ (auto-rollback preserved it)" || echo "v2 down (rollback failed)"
curl -sf http://localhost:8082/health && echo " ← v3 running (unexpected)" || echo "v3 not running ✓ (failed health check, killed)"
...
echo "  2. Rolling redeploy v2 (v1→v2 on :8081)    ✓ zero downtime"
echo "  3. Broken v3 auto-rollback (v2 preserved)  ✓ no downtime"
echo "  4. Recovery redeploy v2 (already running)   ✓"
echo ""
